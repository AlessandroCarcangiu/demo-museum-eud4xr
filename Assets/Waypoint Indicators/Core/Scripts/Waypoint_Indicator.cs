...
public class Waypoint_Indicator : MonoBehaviour
{
    ...
    public bool hidesWhenReachedMinDist = false;
    #endregion
    ...
    public UnityEvent<bool> onScreenEvent;
    public UnityEvent<bool> onWaypointReachedEvent;

    private bool parentOnScreen
    {
        get => _parentOnScreen;
        set { _parentOnScreen = value; onScreenEvent?.Invoke(value); }
    }
    ...
    [SerializeField] private bool _parentOnScreen;
            ...
            if (enableStandardTracking)
            {
                ...
                if (enableSprite)
                {
                    ...
                    if (spriteIndicatorCreated && spriteIndicatorRect != null)
                    {
                        ...
                        if (WaypointUtils.IsWaypointInRangeMinMax(waypointDist, displayRangeMin, displayRangeMax, out WaypointUtils.WaypointDistanceStatus waypointDistance)) //ICON IN RANGE
                        {
                            //SPRITE ON-SCREEN -------------------------------------------------------------
                        ...
                        else //ICON OUT OF RANGE - Si
                        {
                            if (hidesWhenReachedMinDist && waypointDistance == WaypointUtils.WaypointDistanceStatus.BelowRange) // J Added
                            {
                                Debug.LogError("TUTTO OK MI NASCONDO");
                                this.enabled = false;
                                // Destroy this indicator
                                // Destroy(spriteIndicator);
                            }
                            else
                            {
                                spriteIndicatorImage.rectTransform.localScale = new Vector3(0, 0, Sprite_Z_Scale);
                ...
                if (enableGameObject)
                {
                    if (gameObjectIndicatorCreated && gameObjectIndicator != null)
                    {
                        ...
                        if (WaypointUtils.IsWaypointInRangeMinMax(waypointDist, displayRangeMin, displayRangeMax, out WaypointUtils.WaypointDistanceStatus waypointDistance)) //PREFAB IN RANGE
                        {
                ...
                if (enableText)
                {
                    if (waypointTextCreated && textGameObject != null)
                    {
                        ...
                        if (WaypointUtils.IsWaypointInRangeMinMax(waypointDist, displayRangeMin, displayRangeMax, out WaypointUtils.WaypointDistanceStatus waypointDistance)) //TEXT IN RANGE
                        {
                            //GLOBAL-----
                            //FONT
...
static class WaypointUtils
{
    public enum WaypointDistanceStatus
    {
        BelowRange,
        InRange,
        AboveRange
    }
    public static bool IsWaypointInRangeMinMax(float waypointDist, float displayRangeMin, float displayRangeMax, out WaypointDistanceStatus result)
    {
        if (waypointDist <= (displayRangeMin + 1))
        {
            result = WaypointDistanceStatus.BelowRange;
            return false;
        }
        else if (waypointDist >= displayRangeMax)
        {
            result = WaypointDistanceStatus.AboveRange;
            return false;
        }
        else
        {
            result = WaypointDistanceStatus.InRange;
            return true;
        }
    }
