...
public class testecaobjectinfo2 : Singleton<ECAObjectInfo>
{
    ////////////// From Generic Hook //////////////
    ...
    private void Awake()
    {
        ...
        // For each key in d, check if the count of the values is the same in d2
    ...
    public ECAObjectsCapabilties GetAllInfoAboutCurrentECAObjects_Efficient()
{
    ...
    foreach (var subDic in cachedRuleUtilsSubjects.Values)
    {
        foreach (var obj in subDic)
        {
            ...
            if (!allSubjects.Contains(gameObject))
            {
                ...
                actionAttributesCache[gameObject] = GetActionAttributesDictionary(gameObject, cachedRuleUtilsSubjects, subjectType);
    ...
    var allActionAttributes = new Dictionary<string, Dictionary<string, List<Dictionary<string, string>>>>(allSubjects.Count);
    foreach (var s_gO in allSubjects)
    {
        var s_gOName = s_gO.name;
        var actionAttributes = actionAttributesCache[s_gO];

        var actionAttributesJson = new Dictionary<string, List<Dictionary<string, string>>>(actionAttributes.Count);
    ...
    var dropdownOptionsAfterVerbForAllSubjects = new Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>>(allSubjects.Count);
    foreach (var s_gO in allSubjects)
    {
        var s_gOName = s_gO.name;
        var actionAttributes = actionAttributesCache[s_gO];
        ...
        foreach (var kvp in actionAttributes)
        {
            ...
            dropdownByVerb[kvp.Key] = DropdownValueChangedVerb(s_gO, cachedRuleUtilsSubjects, kvp.Value);
        }
    ...
    var dropdownOptionsAfterObjectForAllSubjects = new Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>>>(allSubjects.Count);
    foreach (var s_gO in allSubjects)
    {
        var s_gOName = s_gO.name;
        var actionAttributes = actionAttributesCache[s_gO];

        var verbDict = new Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>>(actionAttributes.Count);
        foreach (var attrList in actionAttributes.Values)
        {
            ...
            foreach (var attr in attrList)
            {
                ...
                {
                    stateVarDict[attr.variableName] = DropdownValueChangedObjectValue(attr.variableName, attrList);
                }
            }
    ...
    public ECAObjectsCapabilties GetAllInfoAboutCurrentECAObjects_Efficient_But_Readible()
    {
        var cachedRuleUtilsSubjects = RuleUtils.FindSubjects();
    ...
    public ECAObjectsCapabilties GetAllInfoAboutCurrentECAObjects_EfficientMia()
    {
        var cachedRuleUtilsSubjects = RuleUtils.FindSubjects();
        ...
        foreach (var s_gO in allSubjects)
        {
            ...
            var actionAttributes = GetActionAttributesDictionary(s_gO, cachedRuleUtilsSubjects, subjectSelectedType);
        ...
        List<ActionAttribute> actionAttributes)
    {
     ...
     private Dictionary<string, List<string>> DropdownValueChangedVerb(GameObject subjectSelected, Dictionary<int,Dictionary<GameObject,string>> subjects,
        List<ActionAttribute> actionAttributes)
    {
        var optionsDic = new Dictionary<string, List<string>>();
