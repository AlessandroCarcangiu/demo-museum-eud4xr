...
public class ECAObjectInfo : Singleton<ECAObjectInfo>
{
    ...
    {
        return s.Split(Convert.ToChar(AgileUtils.AgileSep)).Length != needed ? AgileUtils.PARAMETERS_ERROR : AgileUtils.SUCCESS;
    }
    
    private static Tuple<string, GameObject> ObjectExists(string name)
    {
        var obj = GameObject.Find(name);
        return  obj == null ? new Tuple<string, GameObject>(AgileUtils.OBJECT_NOT_FOUND, null) : new Tuple<string, GameObject>(AgileUtils.SUCCESS, obj);
    }
    ...
    public struct ECAObjectsCapabilties
    {
        ...
        public Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>> allDropdownOptionsAfterVerbs;
        public Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>>> allDropdownOptionsAfterObjects;
    }
    private void Awake()
    {
    ...
    private Dictionary<string, Dictionary<string, List<Dictionary<string, string>>>> GetActionAttributesForAllSubjects_AsDict()
    {
    ...
    /// </summary>
    /// <returns></returns>
    private Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>> GetDropdownOptionsAfterVerbForAllSubjects_AsDict()
    {
        var subjectsName = GetSubjectsNameAsSet();
    ...
    private Dictionary<string, Dictionary<string, Dictionary<string, Dictionary<string, List<string>>>>> GetDropdownOptionsAfterObjectForAllSubjects_AsDict()
    {
        var jsonDropInfo =
        ...
        string subjectName)
    {
        ...
        if (CheckValues(subjectName, 1) != AgileUtils.SUCCESS)
        {
            AgileUtils.AgileErrorStatus(AgileUtils.PARAMETERS_ERROR, "1", subjectName);
            return null;
        }

        var objectCheck = ObjectExists(subjectName);

        if (objectCheck.Item1 == AgileUtils.OBJECT_NOT_FOUND)
        {
            AgileUtils.AgileErrorStatus(AgileUtils.OBJECT_NOT_FOUND, "", subjectName);
            return null;
        }
    ...
    private Dictionary<string, List<Dictionary<string, string>>> GetActionAttributes_AsDictionary(string subjectName)
    {
        var subjects = RuleUtils.FindSubjects();
        ...
        if (!AgileUtils.IsEcaObject(objectCheck.Item2))
        {
            AgileUtils.AgileErrorStatus(AgileUtils.NOT_ECA_OBJECT, "", subjectName);
            return null;
        }
     ...
     */
            ...
            if (ac.variableName == objSelectedString)
            {
                ...
                switch (ac.ValueType.Name)
                {
                    ...
                    case "String":
                        ...
                        {
                            outputDic.Add(ECAUI_Utils.DROPDOWN_5B_INPUT_FIELD, new List<string>() { "alphanumeric" });
                        }
        ...
        if (outputDic.Count < 1)
            AgileUtils.AgileErrorStatus(AgileUtils.CRITICAL_ERROR, "L'oggetto non dispone di actionAttributes!",
                "");
    ...
    public List<string> GetVerbsByObjectName(string subjectName)
    {
        ...
        if (CheckValues(subjectName, 1) != AgileUtils.SUCCESS)
        {
            AgileUtils.AgileErrorStatus(AgileUtils.PARAMETERS_ERROR, "1", "");
            return verbList;
        }
        ...
        if (verbList.Count < 1)
            AgileUtils.AgileErrorStatus(AgileUtils.CRITICAL_ERROR,
