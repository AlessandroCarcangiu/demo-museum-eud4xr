...
{
    [SerializeField] [Tooltip("A mapping between icon names and the unicode value of the glyph it describes.")]
    ...
    [Tooltip("Optional material to use for rendering glyphs in editor.")] [SerializeField]
    ...
    public static uint ConvertHexStringToUnicode(string charString)
    {
        ...
        for (int i = 0; i < charString.Length; i++)
        {
            ...
            if (i < charString.Length - 1 && char.IsHighSurrogate((char)unicode) &&
                char.IsLowSurrogate(charString[i + 1]))
            {
