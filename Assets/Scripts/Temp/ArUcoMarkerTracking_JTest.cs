...
namespace TryAR.MarkerTracking
{
    ...
    /// </summary>
    public class ArUcoMarkerTracking_JTest : MonoBehaviour
    {
        public bool doLog = false; // Enable debug logging

        /// <summary>
        /// Determines whether the GOs associated to the markers should be always active or only at detection. Set to:
        ///  - true (for debugging purposes) to always see the GOs, independent of detection.
        ///  - false to see the GOs only when the markers are detected.
        /// </summary>
        [Tooltip("Determines whether the GOs associated to the markers should be always active or only at detection. Set to:\n" +
                 " - true (for debugging purposes) to always see the GOs, independent of detection.\n" +
                 " - false to see the GOs only when the markers are detected.")]
        public bool virtualObjectsAlwaysActive = false;
        ...
        /// </summary>
        [Range(0, 1)] [SerializeField] private float _poseFilterCoefficient = 0.5f;

        /// <summary>
        /// Division factor for input image resolution. Higher values improve performance but reduce detection accuracy.
        /// </summary>
        [SerializeField] private int _divideNumber = 2; //TODO 0415 J: Credo ci interessi il valore 1 per massima qualità
        ...
        //public GameObject cube_as_marker;
        ...
        public void DetectMarker(WebCamTexture webCamTexture, Texture2D resultTexture = null)
        {
            if (_isReady)
            {
                ...
                // Detect markers
                arucoDetector.detectMarkers(_processingRgbMat, _detectedMarkerCorners, _detectedMarkerIds,
                    _rejectedMarkerCandidates);
                ...
                if (doLog)
                    Debug.Log(
                        $"BBB Detecting markers? DetectMarkeRCorners.Count={_detectedMarkerCorners.Count}, _detectedMarkerIds.total()={_detectedMarkerIds.total()}");
        ...
        public void EstimatePoseCanonicalMarker(MarkerMetadataManager metadataManager, Transform camTransform)
        {
            Dictionary<int, MarkerData> arObjects = metadataManager.MarkerMetadatas;
            Debug.Log("[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] metadataManager.MarkerMetadatas count=" + metadataManager.MarkerMetadatas.Count);
            // List<IotDeviceMarkerData> iotDevices = metadataManager.MarkerMetadatas_iotDevices;

            // Skip if not ready or no markers detected
            if (!_isReady)
            {
                // Debug.Log($"AAAA No markers detected or not ready: isReady={_isReady}, _detectedMarkerCorners is NULL?{_detectedMarkerCorners == null} or empty? {_detectedMarkerCorners.Count == 0}");
                return;
            }
            ...
            foreach (var kvp in arObjects)
            {
                ...
                GameObject obj = kvp.Value.vrObject;
                if (doLog)
                    Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] looping for markerId={markerId}");
                ...
                {
                    currMarkId = (int)_detectedMarkerIds.get(i, 0)[0];
                    if (doLog)
                        Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] currMarkId={currMarkId}");
                    if (currMarkId == markerId)
                    {
                        if (doLog)
                            Debug.Log("[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] marker detected this frame");
                ...
                if (!markerDetectedThisFrame)
                {
                    if (doLog)
                        Debug.Log("[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] marker NOT detected this frame");
                }
                ...
                {
                    if (doLog)
                        Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] obj ({obj}) != null");
                    if (virtualObjectsAlwaysActive)
                    {
                        obj.SetActive(true);
                    }
                    else
                    {
                        if (!markerDetectedThisFrame)
                        {
                            EUD4XR_Player_Object.Instance.ForceTriggerExit(obj);
                        }
                        obj.SetActive(markerDetectedThisFrame);
                    }
                }
                else
                {
                    throw new Exception($"Object with marker ID {markerId} is null. How?");
                }
            }

            if (_detectedMarkerCorners.Count == 0)
            {
                Debug.LogWarning("[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] WARNING No markers detected.");;
                return;
            }
            if (_detectedMarkerCorners == null)
            {
                Debug.LogWarning("[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] WARNING _detectedMarkerCorners is NULL or empty.");;
                return;
            }
                   ...
                   ))
            {
                ...
                {
                    if (doLog) Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] _detectedMarkerCorners.Count="+_detectedMarkerCorners.Count);
                    // Get marker ID
                    int currentMarkerId = (int)_detectedMarkerIds.get(i, 0)[0];
                    if (doLog) Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] currentMarkerId="+currentMarkerId);

                    // Check if this marker has a corresponding game object
                    //logs all arObjects keys
                    foreach (var key in arObjects.Keys)
                    {
                        if (doLog) Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] arObjects key="+key);
                    }

                    var x = arObjects.TryGetValue(currentMarkerId, out MarkerData markerData2);
                    if (doLog) Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] currMarkerId=" +currentMarkerId + "  |  arObjects.TryGetValue returned x="+x);
                    if (doLog) Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] markerData2="+markerData2);
                    
                    if (!arObjects.TryGetValue(currentMarkerId, out MarkerData markerData) || markerData == null)
                        continue;
                    if (doLog) Debug.Log($"[ArUcoMarkerTracking - EstimatePoseCanonicalMarker] markerData.metadata="+markerData.metadata);

                    var targetObject = markerData.vrObject;
                    if (targetObject == null)
                    {
                        throw new Exception(
                            "VR TARGET OBJECT IS NULL. You forgot to assign the prefab in the inspector?");
                    }
                    ...
                    using (MatOfPoint2f imagePoints = new MatOfPoint2f(corner_4x1))
                    {
                        ...
                        // targetObject.transform.localScale = new Vector3(_markerLength, _markerLength, _markerLength); // cylinder
                        targetObject.transform.localScale = markerData.metadata.scale; // cylinder
                        targetObject.transform.localPosition += markerData.metadata.trans_offset; // proviamo qui
                    }
                }
            }
        }
