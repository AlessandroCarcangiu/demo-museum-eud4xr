    ...
    public class StepsManager : MonoBehaviour
    {
        ...
        private void SetOperatorInfo([NotNull] Expression expression, int index)
        {
            if (expression is Order)
            {
                ...
                SetOperatorDescription("All automations must be executed, in any order, to go to the next step");
            }
            else if (expression is Choice)
            {
                SetOperatorName("CHOICE");
                SetOperatorDescription("Automations are mutually exclusive, only one must be executed to go to the next step");
            }
            else
        ...
        private void SetOperatorInfo([NotNull] Automation automation)
        {
            ...
            {
                SetOperatorName("AUTOMATION");
                SetOperatorDescription("The single automation must be executed to go to the next step");
            }
        ...
        private void SetItemInfo([NotNull] Expression expression, int stepIndex)
        {
            ...
            {
                var name = item.Name["automation.".Length..].Replace('_', ' ');
            ...
            else if (expression is Sequence)
            {
                ...
                SetItemInfo($"<size=7>{item.Name[(item.Name.IndexOf(".") + 1)..].Replace('_', ' ')}</size=7>\n\n" +
                    $"<size=6>Subexpression detected, press the label to open/close a temporary card</size=6>");
        }
