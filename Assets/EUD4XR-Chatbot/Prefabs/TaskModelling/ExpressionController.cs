...
{
    public class ExpressionController : Singleton<ExpressionController>
    {
        // public Button leftArrow;
        public MyCustomButton leftArrow;

        // public Button rightArrow;
        public MyCustomButton rightArrow;
        ...
        [Header("Vertical Compact View UI")] public MyCustomButton verticalUpArrow;

        // public Button verticalDownArrow;
        public MyCustomButton verticalDownArrow;

        // public Button toggleViewButton;
        public MyCustomButton toggleViewButton;

        private List<object> exprContent = new List<object>();
        ...
        [SerializeField] private string filename = "expressions-stored.json";

        private Dictionary<string, string>
            automations = new Dictionary<string, string>(); // todo da portare dentro hassunityintegration
        ...
        private void Start()
        {
            StartCoroutine(GetArrowsPosition());
            if (autoSetupWorldSpace)
                SetupWorldSpaceCanvas();
        }

        IEnumerator GetArrowsPosition()
        {
            yield return null;
            // get arrows' initial position
            if (rightArrow != null)
            {
        ...
        int GetListenerCount(Button button)
        {
            // UnityEventBase stores its delegate in a private field called "m_Calls"
            var field = typeof(UnityEngine.Events.UnityEventBase)
                .GetField("m_Calls", BindingFlags.Instance | BindingFlags.NonPublic);

            var invokeCallList = field?.GetValue(button.onClick);
            if (invokeCallList == null) return 0;

            var callsField = invokeCallList.GetType().GetField("m_RuntimeCalls",
                BindingFlags.Instance | BindingFlags.NonPublic);

            var runtimeCalls = callsField?.GetValue(invokeCallList) as System.Collections.IList;
        ...
        public void RenderExpression(JToken expression)
        {
            ClearRender();

            Debug.Log($"ESPRESSIONE RICEVUTA: {expression}");
            ...
            if (expression != null)
            {
                ...
                if (stepExpressionText != null)
                    stepExpressionText.text = $"Espressione {name}";

                if (expression.Type == JTokenType.Object)
                {
                    JObject obj = (JObject)expression;
                    if (obj["order"] != null)
                        exprContent.Add(obj.ToObject<OrderData>());
                    else if (obj["choice"] != null)
                        exprContent.Add(obj.ToObject<ChoiceData>());
                    else if (obj["iteration"] != null)
                        exprContent.Add(obj.ToObject<IterationData>());
                    else if (obj["conditional"] != null)
                        exprContent.Add(obj.ToObject<ConditionalData>());
                    else if (obj["sequence"] != null)
                    {
                        foreach (var item in obj["sequence"])
                        {
                            if (item.Type == JTokenType.String)
                                exprContent.Add(item.ToString());
                        }
                    }
                }
                ...
                {
                    nameExpressionText =
                        nameSequencePrefab.transform.Find("Plate/Text").GetComponent<TextMeshProUGUI>();
                ...
                {
                    stepExpressionText = stepExpressionPrefab.transform.Find("Plate/Text")
                        .GetComponent<TextMeshProUGUI>();
                }

                if (stepExpressionText != null)
                    stepExpressionText.text = "Passo " + (currentIndex + 1) + " di " + exprContent.Count;
            }
            else
            {
            ...
            // show automation
            /*var automation = automations["la_scopa_pulisce_il_tavolo_quando_lo_spruzzino_si_attiva"];
            var card = GameObject.Find("automation.la_scopa_pulisce_il_tavolo_quando_lo_spruzzino_si_attiva");
            AutomationHandler.ShowAutomation(card, automation);*/
            
            
            leftArrow.AddOnClickListener(OnLeftArrow);
            rightArrow.AddOnClickListener(OnRightArrow);

            if (toggleViewButton != null)
            {
                toggleViewButton.AddOnClickListener(() =>
                {
            ...
            if (verticalUpArrow != null)
                verticalUpArrow.AddOnClickListener(ScrollUp);
            if (verticalDownArrow != null)
                verticalDownArrow.AddOnClickListener(ScrollDown);
        ...
        void UpdateStepExpressionText()
        {
            ...
            object current = exprContent.Count > 0 ? exprContent[currentIndex] : null;
        ...
        void UpdateToggleViewButtonActive()
        {
            ...
            if (exprContent.Count == 1 && exprContent[0] is ConditionalData)
            {
                shouldBeActive = true;
            }
            else if (exprContent.Count > 0)
            {
                object current = exprContent[currentIndex];
        ...
        void ShowCurrent()
        {
            // Reset arrows' position
            if (rightArrow != null)
                rightArrow.GetComponent<RectTransform>().anchoredPosition =
                    new Vector2(rightArrowInitialPos.x,
                        rightArrowInitialPos.y); //todo bug source - right arrow overlapping left arrow - maybe here ?
            if (leftArrow != null)
            ...
            else if (current is SequenceData seq)
                currentAutomations.AddRange(seq.sequence);
            ...
            while (bgCount < bgRequired)
            {
                if (idx < 0)
                {
                    for (int i = 0; i < exprContent.Count && bgCount < bgRequired; i++)
                    {
                        if (i == currentIndex) continue;
                        foreach (string bgAuto in AutomationsForBackground(exprContent[i], used))
                        {
                ...
                {
                    foreach (string bgAuto in AutomationsForBackground(exprContent[idx], used))
                    {
                        if (bgCount < bgRequired)
                        {
                            var card = CreateExpression(bgAuto, true);
            ...
            else if (current is ConditionalData conditionalData)
            {
                ...
                else
                {
                    ...
                    var arrowIf = exprIf.transform.Find("B_DownArrow"); //TODO Questo oggetto non esiste più?
                    if (arrowIf != null)
                    {
                    ...
                    var arrowElse = exprElse.transform.Find("B_DownArrow"); //TODO Questo oggetto non esiste più?
                    if (arrowElse != null)
                    {
        ...
        IEnumerator AnimateCompactCardChange(string oldAutomation, string newAutomation, bool toDown)
        {
            ...
            SetCompactControlsActive(true,
                compactIndex > 0,
                (exprContent[currentIndex] is OrderData od && compactIndex < od.order.Count - 1)
                || (exprContent[currentIndex] is ChoiceData cd && compactIndex < cd.choice.Count - 1)
            );

            object curr = exprContent[currentIndex];
        ...
        IEnumerator AnimateCompactStepTransition(bool forward)
        {
            ...
            object newStep = exprContent[currentIndex];
            string newAutomation = "";
            ...
            RectTransform reference = null)
        {
            ...
            // set name
            var automationName = automation.Replace("automation.", "");
            expr.name = automationName;
            ...
            var automationString = HomeAssistantUnityIntegration.Instance.automations[automationName];
            AutomationHandler.ShowAutomation(expr, automationString);
        ...
        void SetExpressionName(GameObject expr, string automation, bool isBackground = false, string prefix = null)
        {
            ...
            if (nameText != null)
            {
                ...
                nameText.text = pfx + displayName.Replace("-", " ").Replace("_", " ");;
            }
        }
        ...
        private bool arrowCooldown = false;

        IEnumerator ArrowCooldown()
        {
            arrowCooldown = true;
            yield return new WaitForSeconds(0.2f); // adjust for input speed
            arrowCooldown = false;
        }
        ...
        void OnRightArrow()
        {
            ...
            if (exprContent.Count == 1 && current is ConditionalData)
            {
                if (conditionalBranchIndex == 0)
                {
            ...
            if (current is ConditionalData && compactView)
            {
                ...
                int pos = logicalSeq.FindIndex(e =>
                    e.branch == conditionalBranchIndex && e.doIdx == conditionalDoIndex);
                if (pos < logicalSeq.Count - 1)
                {
                    ...
                    Debug.Log(
                        $"[OnRightArrow][Conditional][Compact] Avanzo: {oldAutomation} => {newAutomation}, nextBranch={next.branch} nextDoIdx={next.doIdx}");
            ...
            if (compactView)
            {
                Debug.Log($"[OnRightArrow][Compact] currentIndex={currentIndex} sequence.Count={exprContent.Count}");
                if (currentIndex < exprContent.Count - 1)
                    StartCoroutine(AnimateCompactStepTransition(true));
                return;
            }

            object nextObj = exprContent[currentIndex + 1];
            object currentStep = exprContent[currentIndex];

            if (currentStep is OrderData od && compactIndex < od.order.Count - 1)
            {
                Debug.Log(
                    $"[OnRightArrow][OrderData][Compact] compactIndex={compactIndex}, orderCount={od.order.Count}");
                string oldAutomation = od.order[compactIndex];
                string newAutomation = od.order[compactIndex + 1];
                compactIndex++;
                StartCoroutine(AnimateCompactCardChange(oldAutomation, newAutomation, true));
            ...
            {
                Debug.Log(
                    $"[OnRightArrow][ChoiceData][Compact] compactIndex={compactIndex}, choiceCount={cd.choice.Count}");
                string oldAutomation = cd.choice[compactIndex];
                string newAutomation = cd.choice[compactIndex + 1];
                compactIndex++;
                StartCoroutine(AnimateCompactCardChange(oldAutomation, newAutomation, true));
            ...
            else if (currentStep is string && (nextObj is OrderData || nextObj is ChoiceData ||
                                               nextObj is IterationData ||
                                               nextObj is ConditionalData))
            {
                Debug.Log("[OnRightArrow] CardToOrderOrChoiceTransition +1");
                StartCoroutine(CardToOrderOrChoiceTransition(+1, nextObj));
            }
            else if ((currentStep is OrderData || currentStep is ChoiceData || currentStep is IterationData ||
                      currentStep is ConditionalData) && nextObj is string)
            {
                ...
                orderText.gameObject.SetActive(false);
                      ...
                      currentStep is ConditionalData) &&
                     (nextObj is OrderData || nextObj is ChoiceData || nextObj is IterationData ||
                      nextObj is ConditionalData))
            {
        ...
        void OnLeftArrow()
        {
            ...
            if (isAnimating)
            {
                Debug.Log("[OnLeftArrow] isAnimating=true, exit");
                return;
            }

            Debug.LogWarning(exprContent.Count);

            if (exprContent.Count == 0)
            {
                Debug.Log("[OnLeftArrow] sequence.Count==0, exit");
                return;
            }
            ...
            if (current is ConditionalData && compactView)
            {
                ...
                int pos = logicalSeq.FindIndex(e =>
                    e.branch == conditionalBranchIndex && e.doIdx == conditionalDoIndex);
                Debug.Log(
                    $"[OnLeftArrow][Conditional][Compact] branch={conditionalBranchIndex} doIdx={conditionalDoIndex} pos={pos} logicalSeq.Count={logicalSeq.Count}");
                if (pos > 0)
                {
                    ...
                    Debug.Log(
                        $"[OnLeftArrow][Conditional][Compact] Indietro: {oldAutomation} => {newAutomation}, prevBranch={prev.branch} prevDoIdx={prev.doIdx}");
            ...
            if (compactView)
            {
                Debug.Log($"[OnLeftArrow][Compact] currentIndex={currentIndex} sequence.Count={exprContent.Count}");
                if (currentIndex > 0)
                    StartCoroutine(AnimateCompactStepTransition(false));
                return;
            }

            object prevObj = exprContent[currentIndex - 1];
            object currentStep = exprContent[currentIndex];

            if (currentStep is OrderData od && compactIndex > 0)
            {
                Debug.Log(
                    $"[OnLeftArrow][OrderData][Compact] compactIndex={compactIndex}, orderCount={od.order.Count}");
                string oldAutomation = od.order[compactIndex];
                string newAutomation = od.order[compactIndex - 1];
                compactIndex--;
                StartCoroutine(AnimateCompactCardChange(oldAutomation, newAutomation, false));
            ...
            {
                Debug.Log(
                    $"[OnLeftArrow][ChoiceData][Compact] compactIndex={compactIndex}, choiceCount={cd.choice.Count}");
                string oldAutomation = cd.choice[compactIndex];
                string newAutomation = cd.choice[compactIndex - 1];
                compactIndex--;
                StartCoroutine(AnimateCompactCardChange(oldAutomation, newAutomation, false));
            ...
            else if (currentStep is string && (prevObj is OrderData || prevObj is ChoiceData ||
                                               prevObj is IterationData ||
                                               prevObj is ConditionalData))
            {
                Debug.Log("[OnLeftArrow] AnimateDeckInOrderOrChoiceThenExpand -1");
                StartCoroutine(AnimateDeckInOrderOrChoiceThenExpand(prevObj));
            }
            else if ((currentStep is OrderData || currentStep is ChoiceData || currentStep is IterationData ||
                      currentStep is ConditionalData) && prevObj is string)
            {
                ...
                orderText.gameObject.SetActive(false);
                      ...
                      currentStep is ConditionalData) &&
                     (prevObj is OrderData || prevObj is ChoiceData || prevObj is IterationData ||
                      prevObj is ConditionalData))
            {
        ...
        IEnumerator AnimateOrderCardsFromCenter(List<GameObject> cards, float cardWidth, float cardHeight,
            float duration)
        {
            ...
            if (rightArrow != null)
            {
                ...
                if (n >= 2)
                {
                    ...
                    pos.x += cardWidth +
                             maxOrderSpacing; //todo bug source - right arrow overlapping left arrow - maybe here ?
        ...
        void UpdateArrowInteractable()
        {
            ...
            if (exprContent.Count == 1 && exprContent[0] is ConditionalData)
            {
                ...
                if (leftArrow != null) leftArrow.SetInteractable(true);
                // if (rightArrow != null) rightArrow.interactable = true;
                if (rightArrow != null) rightArrow.SetInteractable(true);
                return;
            }

            if (leftArrow != null)
                // leftArrow.interactable = currentIndex > 0;
                leftArrow.SetInteractable(currentIndex > 0);
            if (rightArrow != null)
                // rightArrow.interactable = currentIndex < sequence.Count - 1;
                rightArrow.SetInteractable(currentIndex < exprContent.Count - 1);
        }
        ...
        private void OnGUI()
        {
            // make two on gui button for simulating left and right arrow clicks
            /*if (GUI.Button(new Rect(10, 10, 100, 30), "Left Arrow"))
            {
                leftArrow.SimulateClickForOnGUI();
            }

            if (GUI.Button(new Rect(10, 50, 100, 30), "Right Arrow"))
            {
                rightArrow.SimulateClickForOnGUI();
            }*/
        }
    }
}