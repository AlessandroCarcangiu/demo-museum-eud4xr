...
public class FontIconSetInspector : UnityEditor.Editor
{
    ...
    private const string
        defaultShaderName =
            "TextMeshPro/Distance Field SSD"; // Only used for presentation in inspector, not at runtime.
    ...
    private const string downloadIconFontMessage =
        "For instructions on how to install the HoloLens icon font asset, click the button below.";

    private const string hololensIconFontUrl =
        "https://docs.microsoft.com/windows/mixed-reality/mrtk-unity/features/ux-building-blocks/button";
                ...
                if (showAvailableIcons)
                {
                    if (fontAsset.characterTable.Count == 0)
                    {
                        EditorGUILayout.HelpBox(
                            "No icons are available in this font. The font may be configured incorrectly.",
                            MessageType.Warning);
                    ...
                    if (fontIconSet.GlyphIconsByName.Count > 0)
                    {
                        ...
                        using (new EditorGUILayout.VerticalScope())
                        {
                            ...
                            foreach (IconEntry iconEntry in iconEntries)
                            {
                                ...
                                {
                                    if (GUILayout.Button(" ", GUILayout.MinHeight(buttonHeight),
                                            GUILayout.MaxHeight(buttonHeight), GUILayout.MaxWidth(buttonWidth)))
                                    {
    ...
    /// </summary>
    public static void EditorDrawTMPGlyph(Rect position, uint unicode, TMP_FontAsset fontAsset, bool selected = false,
        Material fontRenderMaterial = null)
    {
    ...
    /// Draws a Text Mesh Pro glyph in the supplied <see cref="Rect"/>.
    /// </summary>
    public static void EditorDrawTMPGlyph(Rect glyphRect, TMP_FontAsset fontAsset, TMP_Character character,
        bool selected = false, Material fontRenderMaterial = null)
    {
        if (Event.current.type == EventType.Repaint)
        {
            try
            {
                ...
                if (elementIndex >= 0)
                {
                    ...
                    Texture2D atlasTexture = fontAsset.atlasTextures.Length > atlasIndex
                    ...
                    if (atlasTexture != null)
                    {
                        ...
                        float scale = Mathf.Min(glyphRect.width, glyphRect.height) / normalizedHeight *
                                      iconSizeMultiplier;
                        ...
                        Graphics.DrawTexture(glyphRect, atlasTexture, texCoords, 0, 0, 0, 0, new Color(1f, 1f, 1f),
                            glyphMaterial);
                    }
                }
            }
            catch (Exception)
            {
                EditorGUILayout.LabelField(
                    "Couldn't draw character icon. UnicodeValue may not be available in the font asset.");
            }
        }
    }
}